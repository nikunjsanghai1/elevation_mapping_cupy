...
class ElevationMap:
    ...
    def __init__(self, param: Parameter, logger=None):
        """
        ...
        self.logger = logger if logger is not None else rclpy.logging.get_logger('elevation_map')
    ...
    def update_map_with_kernel(self, points_all, channels, R, t, position_noise, orientation_noise):
        ...
        with self.map_lock:
            ...
            valid_cells = int(cp.count_nonzero(self.elevation_map[2] > 0.5))
            self.logger.info(f"[DEBUG] valid cells after update: {valid_cells}")
            mask = self.elevation_map[2] > 0.5
            if int(mask.sum()) > 0:
               zvals = self.elevation_map[0][mask]
               zmin = float(zvals.min())
               zmax = float(zvals.max())
               zmean = float(zvals.mean())
               self.logger.info(f"[DEBUG] elevation_map[0]  valid→  min={zmin:.3f}, max={zmax:.3f}, mean={zmean:.3f}")
            else:
               self.logger.warn("[DEBUG] elevation_map[0] has no valid cells!")
    ...
    def input_pointcloud(
        ...
        total_pts = raw_points.shape[0]

        # NaN‐filter
        xyz = raw_points[:, :3]
        nan_mask = ~cp.isnan(xyz).any(axis=1)
        pts_after_nan = int(nan_mask.sum())

        # ramped‐height & range filters
        sx, sy, sz = t.tolist()
        d2   = (xyz[:,0]-sx)**2 + (xyz[:,1]-sy)**2 + (xyz[:,2]-sz)**2
        dxy  = cp.maximum(cp.sqrt(xyz[:,0]**2 + xyz[:,1]**2) - self.param.ramped_height_range_b, 0)
        zrel = xyz[:,2] - sz
        ok1  = d2 >= self.param.min_valid_distance**2
        ok2  = zrel <= (dxy*self.param.ramped_height_range_a + self.param.ramped_height_range_c)
        ok3  = zrel <= self.param.max_height_range
        final_mask = nan_mask & ok1 & ok2 & ok3
        pts_after_all = int(final_mask.sum())

        # print to console
        self.logger.debug(f"[TRACE][input_pointcloud] {pts_after_all}/{total_pts} pts passed "
                 f"(NaN‐filter: {pts_after_nan}, culled by range: {pts_after_all-pts_after_nan})")
        filtered = raw_points[final_mask]
    ...
    def process_map_for_publish(self, input_map, fill_nan=False, add_z=False, xp=cp):
        ...
        # DEBUG: count valid before any masking
        total_cells = self.cell_n * self.cell_n
        valid_pre_mask = int(cp.count_nonzero(~xp.isnan(m)))
        self.logger.info(f"[DEBUG][publish] pre-mask valid cells: {valid_pre_mask}/{total_cells}")
        if fill_nan:
            m = xp.where(self.elevation_map[2] > 0.5, m, xp.nan)
            valid_post_mask = int(cp.count_nonzero(~xp.isnan(m)))
            self.logger.info(f"[DEBUG][publish] post-mask valid cells: {valid_post_mask}/{total_cells}")    
        if add_z:
            m = m + self.center[2]
        return m[1:-1, 1:-1]
