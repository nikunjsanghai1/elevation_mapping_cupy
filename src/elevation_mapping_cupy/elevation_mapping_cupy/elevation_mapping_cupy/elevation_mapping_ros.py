...
class ElevationMappingNode(Node):
    ...
    def initialize_elevation_mapping(self) -> None:
        ...
        self._map = ElevationMap(self.param,logger=self.get_logger())
    ...
    def publish_map(self, key:str) -> None:
        if self._map_q is None:
            self.get_logger().warn(f"[TRACE][publish_map] map_q is None")
            return
        # TRACE: publishing map
        self.get_logger().debug(f"[TRACE][publish_map] key={key} building GridMap msg")        
        gm = GridMap()
        ...
        self.get_logger().debug(f"[TRACE][publish_map] published {key}")        

    def safe_lookup_transform(self, target_frame: str, source_frame: str, time) -> Optional[tf2_ros.TransformStamped]:
        # 1) first check if the transform is available
        if not self._tf_buffer.can_transform(
            target_frame, source_frame, time,
            timeout=Duration(seconds=0.1)
        ):
            self.get_logger().debug(
                f"[TRACE][safe_lookup_transform] waiting for TF "
                f"{source_frame}→{target_frame} @ {time.sec}.{time.nanosec}"
            )
            return None

        # 2) now actually grab it (this should be fast if can_transform succeeded)
        try:
            ...
            self.get_logger().debug(
                f"[TRACE][safe_lookup_transform] got TF "
                f"{source_frame}→{target_frame} @ {tf.header.stamp.sec}.{tf.header.stamp.nanosec}: "
                f"trans=({t.x:.3f},{t.y:.3f},{t.z:.3f})"
            )
            return tf

        # 3) catch & warn if anything goes wrong
        except TransformException as ex:
            self.get_logger().warn(
                f"[TRACE][safe_lookup_transform] TF failed: {ex}"
            )
            return None
    
    def image_callback(self, camera_msg: Image, camera_info_msg: CameraInfo, sub_key: str) -> None:
        # TRACE: incoming image
        self.get_logger().debug(f"[TRACE][image_callback] sub_key={sub_key} header.frame_id={camera_msg.header.frame_id}")
        ...
        try:
            ...
            self.get_logger().warn(f"[TRACE][image_callback] cv_bridge error: {e}")
            return
        # If multi-channel, list them
        shapes = semantic_img.shape
        if len(shapes) != 2:
            semantic_img = [semantic_img[:, :, k] for k in range(shapes[2])]
        else:
            semantic_img = [semantic_img]
        self.get_logger().debug(f"[TRACE][image_callback] semantic_img layers shapes={[img.shape for img in semantic_img]}")
        ...
        self.get_logger().debug(
            f"[TRACE][image_callback] TF {camera_msg.header.frame_id}->" \
            f"{self.map_frame} t={t_np.tolist()} q={[q.x,q.y,q.z,q.w]}"
        )

        # TRACE: before input_image
        self.get_logger().debug(f"[TRACE][image_callback] passing to _map.input_image")
        ...
        self.get_logger().debug(f"[TRACE][image_callback] _image_process_counter={self._image_process_counter}")

    def pointcloud_callback(self, msg: PointCloud2, sub_key: str) -> None:
        # Set the timestamp for transform lookups
        self._last_t = msg.header.stamp
        
        # TRACE: incoming pointcloud
        self.get_logger().debug(
            f"[TRACE][pointcloud_callback] sub_key={sub_key} " \
            f"header.frame_id={msg.header.frame_id} width={msg.width} height={msg.height} point_step={msg.point_step}"
        )

        try:
            points = rnp.numpify(msg)
        except Exception as e:
            self.get_logger().warn(f"[TRACE][pointcloud_callback] numpify error: {e}")
            return
        raw_count = points['xyz'].shape[0]
        self.get_logger().debug(f"[TRACE][pointcloud_callback] raw points count={raw_count}")
        if raw_count == 0:
            return
        
        frame_sensor_id=msg.header.frame_id
        # 2) pull the transform (should always succeed)
        tf = self.safe_lookup_transform(
            self.map_frame,
            frame_sensor_id,
            msg.header.stamp
        ) 
        if tf is None:
            self.get_logger().warn(f"[TRACE][pointcloud_callback] TF is None")
            return
        t = tf.transform.translation
        ...
        R = quaternion_matrix([q.x, q.y, q.z, q.w])[:3, :3].astype(np.float32)
        self.get_logger().debug(
            f"[TRACE][pointcloud_callback] TF {msg.header.frame_id}->{self.map_frame} "
            f"t={t_np.tolist()} q={[q.x,q.y,q.z,q.w]}"
        )
        
        # TRACE: before input_pointcloud
        channels = ["x", "y", "z"] + self.param.subscriber_cfg[sub_key].get("channels", [])
        self.get_logger().debug(
            f"[TRACE][pointcloud_callback] input_pointcloud channels={channels}"
        )
        self._map.input_pointcloud(points['xyz'], channels, R, t_np, 0, 0)
        self._pointcloud_process_counter += 1
        self.get_logger().debug(f"[TRACE][pointcloud_callback] _pointcloud_process_counter={self._pointcloud_process_counter}")

    def pose_update(self) -> None:
        if self._last_t is None:
            self.get_logger().warn(f"[TRACE][pose_update] _last_t is None")
            return
            
        # If base_link and map_frame are the same, create identity transform
        if self.base_frame == self.map_frame:
            self.get_logger().debug(f"[TRACE][pose_update] base_frame and map_frame are the same, using identity transform")
            self._map_t = geometry_msgs.msg.TransformStamped().transform.translation
            self._map_t.x = 0.0
            self._map_t.y = 0.0
            self._map_t.z = 0.0
            self._map_q = geometry_msgs.msg.TransformStamped().transform.rotation
            self._map_q.x = 0.0
            self._map_q.y = 0.0
            self._map_q.z = 0.0
            self._map_q.w = 1.0
            ...
            if transform is None:
                self.get_logger().warn(f"[TRACE][pose_update] Failed to get transform")
                return
